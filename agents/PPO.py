...
class PPO_Agent:
    ...
    def __init__(self, config, evn=None, istest=True, actor_path=None, critic_path=None):
        self.evn=evn
        self.actor = Actor(config).to(config.device)
        self.critic = Critic(config).to(config.device)
        self.actor_optimizer = torch.optim.Adam(
    ...
    def _init_opponent_pool(self):
        """用当前策略初始化对手池"""
        cloned = self._clone_policy()
        self.opponent_pool.append(cloned)
        
    def _clone_policy(self):
        """克隆当前策略的副本"""
        cloned = Actor(self.config).to(self.device)
        cloned.load_state_dict(self.actor.state_dict())
        return cloned
    ...
    def sample_action(self, obs, reward=None, done=False):
        ...
        augmented_obs = self._augment_observation(obs)
         ...
         # 生成动作时考虑对手策略
        if len(self.opponent_pool) > 0 and np.random.rand() < 0.5:
            opponent = random.choice(self.opponent_pool)
            with torch.no_grad():
                opp_obs_tensor = torch.FloatTensor(augmented_obs['opponent_obs']).to(self.device)
                # 添加必要的维度：batch_size=1, sequence=1, channels=1
                opp_obs_tensor = opp_obs_tensor.unsqueeze(0).unsqueeze(0).unsqueeze(0)  # shape: [1,1,1,H,W]
                # 调用对手策略
    ...
    def update(self):
        ...
        # 清空记忆缓冲区
        self.memory.clear()
        if self.sample_count % self.sp_update_interval == 0:
            self._generate_selfplay_data()
        
        # 策略池动态更新
        self._dynamic_pool_update()
        
    def _generate_selfplay_data(self):
        """生成自博弈轨迹数据"""
        with torch.no_grad():
            for opponent in self.opponent_pool:
                # 与历史策略对抗
                traj = self._run_selfplay_episode(opponent)
                self._store_sp_trajectory(traj)
    
    def _run_selfplay_episode(self, opponent):
        
        """运行单次自博弈对局"""
        """运行自博弈回合的完整流程"""
        if self.env is None:
            raise ValueError("Environment not initialized. Call set_env() first.")
        state = self.env.reset()
        episode_data = []
        
        for _ in range(self.config.max_steps):
            # 当前策略动作
            current_action = self.act(state)
            # 对手策略的动作（从池中随机选择）
            opp_action = opponent.act(state)
            
            # 环境交互
            next_state, reward, done, _ = self.env.step([current_action, opp_action])
            
            # 存储轨迹数据
            episode_data.append({
                'state': state,
                'action': current_action,
                'reward': reward[0],  # 当前智能体的奖励
                'done': done
            })
    ...
    def _store_sp_trajectory(self, traj):
        """存储自博弈轨迹到记忆池"""
        for t in traj:
            self.memory.push((
                t['state'], 
                t['action'],
                self._estimate_log_prob(t['action']),
                t['reward'],
                t['done']
            ))
            
    def _dynamic_pool_update(self):
        """基于胜率动态更新对手池"""
        if len(self.opponent_pool) < 2:
            return
            
        # 评估当前策略对历史策略的胜率
        win_rate = self._evaluate_against_pool()
        
        # 胜率超过阈值时更新池
        if win_rate > self.sp_win_rate_threshold:
            self.opponent_pool.append(self._clone_policy())
            print(f"策略池更新，当前容量: {len(self.opponent_pool)}")
    ...
    def _evaluate_against_pool(self):
        ...
        for _ in range(total_episodes):
            opponent = random.choice(self.opponent_pool)
            traj = self._run_selfplay_episode(opponent)
            final_reward = sum(t['reward'] for t in traj)
            if final_reward > 0:  # 根据环境定义调整阈值
                win_counts += 1
                
        return win_counts / total_episodes
