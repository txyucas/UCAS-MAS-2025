...
class billiard_competition(OlympicsBase):
    def __init__(self, map):
        ...
        self.vis = [self.agent_list[0].visibility, self.agent_list[1].visibility]
        self.vis_clear = [self.agent_list[0].visibility_clear, self.agent_list[1].visibility_clear]
    ...
    def reset_cure_ball(self):      #fixme : random reset, need to check for overlap as well
        ...
        for idx in new_agent_idx:
            ...
            new_agent = Agent(mass = 1, r = 15, position = [random_init_pos_x, random_init_pos_y],
                              color = self.white_ball_color[idx], vis = self.vis[idx], vis_clear = self.vis_clear[idx])
            self.agent_list.append(new_agent)
            self.white_ball_in[idx] = False

            new_boundary = self.get_obs_boundaray([random_init_pos_x, random_init_pos_y], 15, self.vis[idx])
    ...
    def get_reward(self):
        ...
        for obj in self.map['objects']:
            if obj.can_pass() and obj.color == self.cross_color:
                center_x = obj.init_pos[0] + 0.5 * obj.init_pos[2] 
                center_y = obj.init_pos[1] + 0.5 * obj.init_pos[3]
                pocket_positions.append([center_x, center_y])

        # 计算每个白球到最近球袋的距离，并添加惩罚
        for idx, agent in enumerate(self.agent_list):
            if agent.type == 'agent':
                agent_pos = self.agent_pos[idx]
                min_dist = float('inf')

                # 找到最近的球袋
                for pocket_pos in pocket_positions:
                    dist = math.sqrt(
                        (agent_pos[0] - pocket_pos[0])**2 + 
                        (agent_pos[1] - pocket_pos[1])**2
                    )
                    min_dist = min(min_dist, dist)

                # 当距离小于安全距离时添加惩罚
                safe_distance = 40  # 可以调整这个安全距离
                if min_dist < safe_distance:
                    penalty = -3 * (1 - min_dist/safe_distance)  # 距离越近惩罚越大
                    if agent.color == self.agent1_color:
                        reward[0] += penalty
                    else:
                        reward[1] += penalty
        ...
        for idx, agent in enumerate(self.agent_list):
            if agent.type == 'agent':
                white_ball_indices.append(idx)

        # 初始化位置记录
        if not hasattr(self, 'agent_last_pos'):
            self.agent_last_pos = copy.deepcopy(self.agent_pos)

        # 计算所有物体的速度
        for idx in range(self.agent_num):
            velocity = [
                self.agent_pos[idx][0] - self.agent_last_pos[idx][0],
                self.agent_pos[idx][1] - self.agent_last_pos[idx][1]
            ]
            self.agent_list[idx].velocity = velocity

        # 记录当前位置用于下一帧计算
        self.agent_last_pos = copy.deepcopy(self.agent_pos)

        # 对每个白球检查其碰撞
        for white_idx in white_ball_indices:
            if hasattr(self, f'last_velocity_{white_idx}'):
                current_velocity = self.agent_list[white_idx].velocity
                last_velocity = getattr(self, f'last_velocity_{white_idx}')
                velocity_change = [
                    current_velocity[0] - last_velocity[0],
                    current_velocity[1] - last_velocity[1]
                ]

                # 检查与其他球的碰撞
                for other_idx in range(self.agent_num):
                    if other_idx != white_idx and self.agent_list[other_idx].type == 'ball':
                        distance = math.sqrt(
                            (self.agent_pos[white_idx][0] - self.agent_pos[other_idx][0])**2 +
                            (self.agent_pos[white_idx][1] - self.agent_pos[other_idx][1])**2
                        )

                        if distance <= (self.agent_list[white_idx].r + self.agent_list[other_idx].r + 1):
                            # 确定是哪个玩家的白球
                            player_idx = 0 if self.agent_list[white_idx].color == self.agent1_color else 1
                            other_ball_color = self.agent_list[other_idx].original_color

                            # 判定有效碰撞：白球击中己方颜色的球
                            impact_speed = math.sqrt(velocity_change[0]**2 + velocity_change[1]**2)
                            if (player_idx == 0 and other_ball_color == self.agent1_ball_color) or \
                               (player_idx == 1 and other_ball_color == self.agent2_ball_color):
                                collision_reward[player_idx] = min(0.1 * impact_speed, 0.3)
                            # 击中对方的球，给予较小惩罚
                            else:
                                collision_reward[player_idx] = -0.1

            # 保存当前速度用于下一帧比较
            setattr(self, f'last_velocity_{white_idx}', self.agent_list[white_idx].velocity.copy())
