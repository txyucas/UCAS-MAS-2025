    ...
    def get_reward(self):
        ...
        for agent_idx in range(self.agent_num):
            ...
            if agent.type == 'ball':
                ball_idx = agent_idx
                break
        if not hasattr(self, 'agent_init_pos'):  # 检查是否初始化了agent_init_pos
                self.agent_init_pos = self.agent_pos  # 初始化为适当大小的列表
        # 如果能找到球,计算球的横向移动
        if ball_idx is not None:
            current_x = self.agent_pos[ball_idx][0]  # 当前x位置
            if hasattr(self, 'last_ball_x'):  # 如果存在上一步的位置
                dx = current_x - self.last_ball_x  # 计算x方向位移
                if hasattr(self, 'dx'):
                    if abs(dx) > abs(self.dx):
                        if dx < 0:  # 球向左移动
                            if self.agent_init_pos[0][0] > 400:  # 第一个智能体的初始位置在右边
                                movement_reward[0] = min(-dx * 0.01, 0.05)  # 根据位移大小给予奖励,但不超过0.1
                                movement_reward[1] = -0.2*movement_reward[0]
                            else:  # 第二个智能体的初始位置在右边
                                movement_reward[1] = min(-dx * 0.01, 0.05)
                                movement_reward[0] = -0.2*movement_reward[1]
                        elif dx > 0:
                            if self.agent_init_pos[0][0] < 400:  # 第一个智能体的初始位置在左边
                                movement_reward[0] = min(dx * 0.01, 0.05)
                                movement_reward[1] = -0.2*movement_reward[0]
                            else:
                                movement_reward[1] = min(dx * 0.01, 0.05)
                                movement_reward[0] = -0.2*movement_reward[1]
                self.dx = dx  # 更新dx为当前的位移
            self.last_ball_x = current_x  # 保存当前位置作为下一步的last_ball_x
