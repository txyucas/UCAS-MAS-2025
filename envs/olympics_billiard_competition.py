...
class OlympicsBilliardCompetition(Game):
    def __init__(self, conf, seed=None):
        super(OlympicsBilliardCompetition, self).__init__(conf['n_player'], conf['is_obs_continuous'], conf['is_act_continuous'],
                                         conf['game_name'], conf['agent_nums'], conf['obs_type'])
        self.seed = seed
        self.set_seed()

        Gamemap = create_scenario("billiard-competition")
        self.env_core = billiard_competition(Gamemap)
